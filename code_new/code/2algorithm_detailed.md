...
$$f_1 = \sum_{k=1}^{K} \sum_{(i,j) \in \mathrm{route}_k} d_{ij}$$
...
$$f_2 = \frac{1}{n} \sum_{k=1}^{K} \sum_{i \in \mathrm{route}_k} w_i, \quad w_i = \max(0, \; e_i - a_i)$$
...
**Công thức thay thế (Eq. 5):** $f_3 = \sum_k |T_k - T_{\mathrm{avg}}|$ — tổng độ lệch tuyệt đối so với trung bình. Dùng khi muốn phạt cả route dài lẫn route ngắn bất thường.

### 1.3. Ràng buộc cứng

| Ràng buộc | Công thức | Ý nghĩa thực tế |
|-----------|-----------|------------------|
| **Tải trọng** | $\sum_{i \in \mathrm{route}_k} q_i \leq Q$ | Xe không được chở quá tải |
...
**Bước 3 — Auto-Calibration:** Sau khi có quần thể ban đầu, hệ thống phân tích giá trị 3 mục tiêu của tất cả lời giải khả thi, tính ideal point (min mỗi mục tiêu) và phân vị thứ 10. Điểm tham chiếu $g$ được hiệu chỉnh: $g_i = \mathrm{ideal}_i + 0.1 \times (p_{10,i} - \mathrm{ideal}_i)$. Bước này đảm bảo $g$ vừa "tham" (gần ideal) nhưng vẫn "khả thi" (dựa trên phân bố thực tế), tránh trường hợp DM đặt $g$ quá lý tưởng khiến ASF vô nghĩa.

**Bước 4 — Vòng lặp Tiến hóa:** Đây là phần cốt lõi, chạy cho đến khi hết ngân sách thời gian $t_{\mathrm{run}}$ (thường 60s). Mỗi thế hệ gồm 8 bước tuần tự:
...
- **(4h)** Adaptive Control kiểm tra trì trệ và điều chỉnh $p_{\mathrm{abs}}$, $p_m$
...
**Smart Route Merging:** Kiểm tra $\mathrm{avg\_utilization} = \frac{\sum q_i}{K \times Q}$. Nếu $< 60\%$ (xe còn dư nhiều tải), thử gộp route ngắn nhất vào các route khác. Nếu tất cả khách hàng trong route ngắn đều chèn được và tổng quãng đường tăng $\leq 5\%$ → chấp nhận gộp → giảm 1 xe. Threshold 5% vì giảm 1 xe tiết kiệm chi phí cố định lớn (lương, bảo hiểm, khấu hao).

**Reverse Encoding:** Cuối cùng, tất cả lời giải dạng route được chuyển ngược về dạng random-key vector $[0,1)^{n_{\mathrm{var}}}$ bằng công thức $\mathrm{keys}[z_i-1] \sim \mathcal{U}(i/n_{\mathrm{var}}, (i+1)/n_{\mathrm{var}})$ — để SSO có thể thao tác trên không gian liên tục.
...
**Bước 8 — Adaptive Control:** Kiểm tra $|\mathrm{ASF}_{\mathrm{best}}^{(t)} - \mathrm{ASF}_{\mathrm{best}}^{(t-1)}| < 10^{-6}$ liên tiếp $\geq 5$ thế hệ? Nếu có → trì trệ → tăng $p_{\mathrm{abs}}$ (nhiều ABS hơn) và $p_m$ (nhiều mutation hơn) → kích hoạt exploration mạnh. Nếu không → giữ tham số bình thường → exploitation ổn định.
...
- **Worst Removal (40%):** Tính $\mathrm{saving}(c_i) = d_{\mathrm{prev},c_i} + d_{c_i,\mathrm{next}} - d_{\mathrm{prev},\mathrm{next}}$ cho mỗi khách hàng — saving lớn = khách hàng "lạc lõng" so với route, loại bỏ sẽ tiết kiệm khoảng cách nhiều nhất. Sắp xếp theo saving giảm dần, loại 15–35% khách hàng (tỷ lệ random trong khoảng). Chiến lược này **targeted** — chỉ loại khách hàng gây tốn chi phí, giữ lại "khung sườn" tốt của route.

- **Route Removal (60%):** Loại $\lfloor |\mathrm{routes}| / 3 \rfloor$ route, xác suất chọn $P(k) \propto \exp(-10 \cdot n_k / n_{\max})$ — route ngắn (ít khách hàng) bị chọn gần như chắc chắn (hệ số $-10$ rất lớn). Chiến lược này **structural** — phá hủy cấu trúc route, tạo cơ hội "xóa bỏ" route thừa. Ví dụ: route chỉ có 1–2 khách hàng thường là kết quả của split không tối ưu — loại bỏ để phân lại vào route khác giúp giảm tổng số xe.
...
2. **Với mỗi khách hàng ứng viên**, tính 3 thành phần:
   ...
   - $\hat{\mathrm{tw}}(c_i)$: mức "gấp" của time window — TW hẹp (deadline sớm) → ưu tiên cao → liên quan đến $f_2$ (waiting)
...
3. **Composite score:** $f(c_i) = w_1 \cdot \hat{d} + w_2 \cdot \hat{\mathrm{tw}} + (1-w_1-w_2) \cdot \hat{h}$ — trọng số $w$ lấy trực tiếp từ preference của DM. Nếu DM muốn distance quan trọng nhất ($w_1 = 0.5$), route sẽ ưu tiên chọn khách gần. Nếu DM muốn cân bằng ($w_3$ cao), route sẽ ưu tiên giữ reachability cao.
...
**Giám sát — ASF best tracking:** Mỗi thế hệ $t$, hệ thống ghi nhận $\mathrm{ASF}_{\mathrm{best}}^{(t)} = \min_{x \in \mathrm{population}} \mathrm{ASF}(x, g, w)$ — giá trị ASF nhỏ nhất trong quần thể hiện tại. ASF được chọn làm metric giám sát (thay vì HV hay IGD) vì: (a) ASF tính nhanh $O(N \cdot M)$, (b) ASF trực tiếp đo "gần preference bao nhiêu" — đúng mục tiêu của preference-based optimization, (c) ASF không cần true Pareto front (IGD cần).

**Phát hiện trì trệ — Stagnation Detection:**
- So sánh $|\mathrm{ASF}_{\mathrm{best}}^{(t)} - \mathrm{ASF}_{\mathrm{best}}^{(t-1)}|$ với threshold $\epsilon_{\mathrm{stag}} = 10^{-6}$.
- Nếu $\geq \epsilon_{\mathrm{stag}}$ (có cải thiện): reset `stag_counter = 0`. Thuật toán đang hội tụ bình thường → giữ tham số ở mức **exploitation** ($p_{\mathrm{abs}} = 0.20$, $p_m = 0.05$) để không phá vỡ quá trình hội tụ tốt.
- Nếu $< \epsilon_{\mathrm{stag}}$ (không cải thiện): tăng `stag_counter += 1`. Đây là dấu hiệu quần thể đang bị **kẹt tại local optima** hoặc đã **hội tụ sớm** (premature convergence).
...
**Giải pháp:** Dùng **mã hóa khóa ngẫu nhiên (Random-Key)** — mỗi lời giải là vector thực $X = (x_1, \ldots, x_{n_{\mathrm{var}}}) \in [0,1)^{n_{\mathrm{var}}}$, với:
...
2. Mọi vector $[0,1)^{n_{\mathrm{var}}}$ đều decode được → không có lời giải "bất hợp lệ" ở mức cấu trúc
3. Dễ perturbation bằng cộng nhiễu Gaussian

### 2.2. Quá trình Decode

**Bước 1 — Sắp xếp:** Tính chỉ số sắp xếp $Z = \mathrm{argsort}(X) + 1$
...
$$\mathrm{keys}[z_i - 1] \sim \mathcal{U}\left(\frac{i}{n_{\mathrm{var}}}, \frac{i+1}{n_{\mathrm{var}}}\right)$$

**Ý nghĩa:** Vị trí thứ $i$ trong chuỗi Z nhận key nằm trong khoảng $[i/n_{\mathrm{var}}, (i+1)/n_{\mathrm{var}})$. Điều này đảm bảo argsort sẽ tái tạo đúng chuỗi Z, đồng thời thêm tính ngẫu nhiên nhỏ (uniform trong mỗi khoảng) để tránh keys trùng lặp.
...
$$\mathrm{ASF}(x, g, w) = \max_{i=1}^{3} \left\{ w_i \cdot (f_i(x) - g_i) \right\}$$
...
3. **Phát hiện trì trệ:** Nếu $\min(\mathrm{ASF})$ không cải thiện qua 5 thế hệ → tăng ABS probability, tăng mutation
4. **Theo dõi hội tụ:** Log $\min(\mathrm{ASF})$ mỗi thế hệ thay vì IGD (vì IGD cần true Pareto front)

### 3.4. Region of Interest (ROI)

$$\mathrm{ROI}: \quad \sum_{i=1}^{3} \left( \frac{w_i \cdot (f_i(x) - g_i)}{\delta \cdot (\mathrm{nadir}_i - \mathrm{ideal}_i)} \right)^2 \leq 1$$
...
- Chuẩn hóa bởi $(\mathrm{nadir}_i - \mathrm{ideal}_i)$ — để vùng ROI có kích thước tương đối, không phụ thuộc đơn vị
...
$$g_i = \mathrm{ideal}_i + 0.1 \times \max(p_{10,i} - \mathrm{ideal}_i, \; 0)$$

- $\mathrm{ideal}_i$ = giá trị tốt nhất mục tiêu $i$ trong quần thể (chỉ tính lời giải khả thi)
...
**Đảm bảo:** $g_i \geq \mathrm{ideal}_i + 0.01 \times \mathrm{range}_i$ — $g$ luôn cao hơn ideal ít nhất 1% range để ASF có gradient rõ ràng.
...
**Ngân sách thời gian:** $\min(40\% \times t_{\mathrm{run}}, 15\mathrm{s})$
...
- $0.5 \times d_{\mathrm{prev},i}$ = khoảng cách (nhỏ → gần)
...
$$x_j^{\mathrm{new}} = \mathrm{clip}(x_j + \varepsilon_j, \; 0, \; 0.999), \quad \varepsilon_j \sim \mathcal{U}(-\sigma, \sigma)$$
...
**Khi áp dụng:** Chỉ khi $\mathrm{avg\_utilization} = \frac{\sum q_i}{K \times Q} < 60\%$ (xe còn dư nhiều tải).
...
$$\mathrm{CD}_i = \sum_{m=1}^{M} \frac{f_m^{i+1} - f_m^{i-1}}{f_m^{\max} - f_m^{\min}}$$
...
$$x_{i,j}^{\mathrm{new}} = \begin{cases}
g_{\mathrm{best},j} & \mathrm{prob.}\; c_g = 0.95 \\
x_{i,j} & \mathrm{prob.}\; c_w - c_g = 0.04 \\
\mathcal{U}(0,1) & \mathrm{prob.}\; 1 - c_w = 0.01
\end{cases}$$
...
$$\mathrm{saving}(c_i) = d_{\mathrm{prev}, c_i} + d_{c_i, \mathrm{next}} - d_{\mathrm{prev}, \mathrm{next}}$$

**Trực quan:** $\mathrm{saving}(c_i)$ = khoảng cách tiết kiệm nếu bỏ qua $c_i$ và nối prev-next trực tiếp. Khách hàng có saving lớn = "lạc lõng" so với route → loại bỏ để chèn lại chỗ tốt hơn.

#### Route Removal (60% xác suất)

Loại bỏ $\lfloor |\mathrm{routes}| / 3 \rfloor$ route, xác suất chọn tỷ lệ nghịch với độ dài:

$$P(\mathrm{route}\; k) \propto \exp\left(-10 \cdot \frac{n_k}{n_{\max}}\right)$$
...
$$h(c_i) = |\{c_j : c_j \;\mathrm{reachable\;after}\; c_i\}|$$

Tức là: $h(c_i)$ = số khách hàng còn lại có thể phục vụ SAU KHI phục vụ $c_i$.
...
$$f(c_i) = w_1 \cdot \hat{d}(c_i) + w_2 \cdot \hat{\mathrm{tw}}(c_i) + (1 - w_1 - w_2) \cdot \hat{h}(c_i)$$

| Thành phần | Ký hiệu | Ý nghĩa | Liên quan mục tiêu |
|------------|---------|----------|---------------------|
| Khoảng cách | $\hat{d}$ | Gần = tốt → giảm $f_1$ | $f_1$ (distance) |
| TW urgency | $\hat{\mathrm{tw}}$ | TW hẹp = ưu tiên → giảm $f_2$ | $f_2$ (waiting) |
...
$$\mathrm{box}(x) = \left\lfloor \frac{f_i(x)}{\varepsilon} \right\rfloor$$
...
$$\mathrm{stagnating} \iff |\mathrm{ASF}_{\mathrm{best}}^{(t)} - \mathrm{ASF}_{\mathrm{best}}^{(t-1)}| < 10^{-6} \;\mathrm{for}\; \geq 5 \;\mathrm{consecutive\;generations}$$
...
|---------|------------|--------------|----------|
| $p_{\mathrm{abs}}$ | 0.20 | $\min(0.50, 0.20 + 0.05 \times \mathrm{stag})$ | Tăng ABS → phá-xây nhiều hơn → thoát local optima |
| $p_m$ | 0.05 | $0.05 + 0.10 \times \mathrm{progress}$ | Tăng mutation → thêm biến thể mới |
...
$$x \;\mathrm{R\text{-}dominates}\; y \iff \begin{cases}
x \succ y \;(\mathrm{Pareto}), & \mathrm{OR} \\
x \in \mathrm{ROI} \land y \notin \mathrm{ROI}, & \mathrm{OR} \\
\mathrm{same\;ROI\;status} \land \mathrm{ASF}(x) < \mathrm{ASF}(y)
\end{cases}$$
...
| **Best ASF** | $\min \mathrm{ASF}(x)$ | Lời giải **gần $g$ nhất** → đo preference satisfaction |
...
$$\mathrm{Cov}(A, P^*) = \frac{|\{v \in P^* : \exists v' \in A, \; v' \preceq v\}|}{|P^*|}$$
...
$$\mathrm{IGD}(A, P^*) = \frac{1}{|P^*|} \sum_{v \in P^*} \min_{v' \in A} \|v - v'\|_2$$
...
$$\mathrm{HV}(A, r) = \mathrm{Vol}\left(\bigcup_{v \in A} \prod_{i=1}^{M} [v_i, r_i]\right)$$

trong đó $r = [r_1, r_2, r_3]$ là **reference point** (thường lấy nadir + margin), $\mathrm{Vol}$ = thể tích M-chiều.
...
**Chuẩn hóa:** Trước khi tính HV, chuẩn hóa mỗi mục tiêu về $[0, 1]$: $\hat{f}_i = \frac{f_i - \mathrm{ideal}_i}{\mathrm{nadir}_i - \mathrm{ideal}_i}$. Reference point $r = [1.1, 1.1, 1.1]$ (10% ngoài nadir). Chuẩn hóa đảm bảo HV không bị chi phối bởi mục tiêu có scale lớn (ví dụ distance ~800 vs waiting ~0.5).
...
$$\mathrm{R\text{-}HV}(A, r, \mathrm{ROI}) = \mathrm{HV}(\{v \in A : v \in \mathrm{ROI}\}, r)$$
...
$$\mathrm{Best\;ASF} = \min_{x \in A} \mathrm{ASF}(x, g, w) = \min_{x \in A} \max_{i=1}^{3} \left\{ w_i \cdot (f_i(x) - g_i) \right\}$$
...
$$\mathrm{ROI\;Count} = |\{x \in A : x \in \mathrm{ROI}\}|$$

trong đó ROI (Region of Interest) được xác định bởi:

$$\sum_{i=1}^{3} \left( \frac{w_i \cdot (f_i(x) - g_i)}{\delta \cdot (\mathrm{nadir}_i - \mathrm{ideal}_i)} \right)^2 \leq 1$$
...
| $t_{\mathrm{run}}$ | 60s | Thời gian chạy tổng |
| $c_g = 0.95$ | Rất cao | Exploitation mạnh — copy gBest |
| $c_w = 0.99$ | Cao | Ít exploration ngẫu nhiên |
| $p_{\mathrm{abs}} = 0.20$ | Trung bình | 20% lần dùng ABS, 80% dùng SSO |
...
| SSO update | $O(N \cdot n_{\mathrm{var}})$ | Vectorized, rất nhanh |
| ABS search | $O(p_{\mathrm{abs}} N n^2)$ | Đắt nhất — rebuild $O(n^2)$ |
| Archive update | $O(|A| \cdot N)$ | Pairwise check |
| **Tổng** | **$O(MN^2 + p_{\mathrm{abs}}Nn^2)$** | Với n=100, N=100 → ~200K ops |
